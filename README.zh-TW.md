...
預設是有損的，因為卡片是拿來看的衍生作品，原檔仍留在你的圖庫裡。以一個 33MP 的相機檔為例，預設會從 18.8 MB 產出 6.8 MB，偏差在任何觀看尺度下都看不出來，並且沿用相機的色度取樣，所以拍成 4:2:2 的機身不會被悄悄砍成 4:2:0。（[量測數據](docs/design.md#encoding-defaults)，英文。）

`--quality` 會直接傳給該格式的編碼器，所以刻度因格式而異、數字之間不可比較——`heic 70` 大約相當於 `jpg 95`。

`--lossless` 用 `jpegtran` 在 DCT 係數層合成，照片的係數原封不動地搬進卡片，照片區與來源位元完全相同。它要求照片的長寬都是 16 的倍數，做不到時會明確報錯，而不是靜默退回。（[為何需要外部二進位](docs/design.md#lossless-compositing)，英文。）
...
**[Design notes](docs/design.md)**（英文）逐一說明上述每個選擇，並附上背後的量測數據。
