...
單位是設計像素而不是輸出像素，所以這些數字在任何卡片尺寸下都成立。它是寬度不是字數，因為根本不存在穩定的字數：`Fushimi Inari, Kyoto` 在最緊的那一欄佔掉 282 之中的 208，還能再放一個短詞；但同樣二十個中文或日文字則完全塞不下。地點超出預算時該張照片會失敗並報出差多少，讓你知道要改哪一行，而不是把字印在簽名上。
